...
from property_based_tester.robot_test_definition_language.textx_test_specification import RobotTestDefinitionLanguageRandomizer
...
pblg = RobotTestDefinitionLanguageRandomizer()
rtdl_config = pblg.rando_scenario  
...
class TestScenario(Base):
    ...
    def test_scenario_generation(self, randomizer, rtdl_config):
        ...
        print(rtdl_config[0])
        world = Model(rtdl_config[0],0,0,0)
        world.insert_model()
        ...
        for o in range(0,5):
            ...
            if is_spawned != True:
                store.append([x,y]) 
                safety_obstacle = Model(random.choice(rtdl_config[1]),x, y, z=0.05, Y=randomizer(-360,360))
    ...
    def test_must_have_collision_force_less_than(self, rtdl_config):
        ...
        """ 
        check = False
        for configuration in rtdl_config[2]:
    ...
    def test_static_obstacle_generation_tear_down(self, rtdl_config):
        ...
        """  
        delete_model(rtdl_config[0]) 

        try:
            for i in rtdl_config[1]:
                delete_model(i) 
        except:
            pass
